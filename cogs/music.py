...
class Music(commands.Cog):
    ...
    async def add_to_queue(self, ctx, query: str):
        ...
        if batch:
            ...
            await ctx.send("Playlist ajoutée à la file d'attente") if len(
                batch
            ) > 1 else await ctx.send("Lien youtube ajouté à la file d'attente")
            ...
            async with ctx.typing():
                if self.ready_queue:
                    ctx.voice_client.play(
                        ...
                        after=lambda e: print(e)
                        if e
                        else self.bot.loop.create_task(self.play_next(ctx)),
                    )
    ...
    @commands.command(
        help="Permet de couper court à la musique en cours et de passer à la suivante"
    )
    ...
    @commands.command(
        help="Permet d'envoyer le bot jouer une musique à quelqu'un d'un autre salon vocal"
    )
            ...
            for chan in guild.channels:
                if channel == chan.name and isinstance(chan, discord.VoiceChannel):
                    if len(chan.members) > 0:
                        ...
                        await ctx.send(
                            f"Il n'y a personne dans le chat vocal {chan.name}"
                        )
                        return
